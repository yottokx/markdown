...
プロジェクトのルートで実行します。Python環境と依存関係はuvで管理します。
...
段階5〜6まで実装済みです。残る段階7では配布の仕上げとPyInstallerのビルド・配布環境での検証を行います。数式・Mermaidの専用エディタは追加しません。

**PyInstallerの再ビルドは段階7まで行いません。現在の `dist` に残るEXEは段階1〜2の旧版です。今回の機能は `uv run md-editor` で確認してください。** 最終配布時の関連付け起動用ファイル引数は対応済みで、アプリ内の関連付け設定機能は設けません。
