...
class MappedText(str):
    ...
    def replace_slice(self, start, end, value):
        """Keep a rewritten URL tied to its original source interval."""
        replacement = MappedText(value, (source_span(self[start:end]),) * len(value))
        return self.concat((self[:start], replacement, self[end:]))
...
def selection_mapping_plugin(parser):
    ...
    def normalize(state):
        original = state.env.get("selection_original", state.src)
        state.env["selection_mapping"] = SelectionMapping(original)
        mapped = state.src if isinstance(state.src, MappedText) else MappedText.original(state.src)
        state.src = mapped.replace("\r\n", "\n").replace("\r", "\n").replace("\0", "\ufffd")
