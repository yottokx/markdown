...
@pytest.mark.parametrize("image", ["![](a.png)", '<img src="a.png">'])
def test_url_rewrite_maps_empty_alt_image_and_following_text_to_original(image):
    from md_editor.assets import rewrite_destinations
    from md_editor.selection_mapping import MappedText

    source = f"😀 {image} **after**"
    normalized = rewrite_destinations(
        MappedText.original(source), lambda _destination: "../long%20folder/image.png?v=1&size=2"
    )
    rendered = render_markdown(normalized, original_source=source)
    elements = SelectionElements(rendered.html).elements
    image_entry = next(
        entry for entry in rendered.selection_map if elements[entry["id"]]["tag"] == "img"
    )
    assert source_slice(source, image_entry["start"], image_entry["end"]) == "a.png"
    text_entry = next(
        entry for entry in rendered.selection_map if elements[entry["id"]]["text"] == "after"
    )
    _lo, _hi, start, end = text_entry["segments"][0]
    assert source_slice(source, start, end) == "after"
    assert "../long%20folder/image.png" in rendered.html
