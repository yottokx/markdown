...
def test_notebook_tabs_use_current_font_and_zoom_for_existing_documents(qtbot, make_notebook):
    window = make_notebook()
    body = "折り返しを使ってノートのフォント変更を確認します。 " * 80
    first = notebook_integration.new_note(qtbot, window, body)
    ready(qtbot, window)
    first_height = window.editor.document().firstBlock().layout().boundingRect().height()
    second = notebook_integration.new_note(qtbot, window, body)
    ready(qtbot, window)
    notebook_integration.saved(qtbot, window)
    revisions = {note: window.store.get(note).revision for note in (first, second)}
    documents = {note: window._sessions[note].document for note in (first, second)}
    undo_steps = {note: document.availableUndoSteps() for note, document in documents.items()}

    window.set_source_font(QFont("Yu Gothic", 19))
    window.set_preview_zoom(1.75)
    for note in (first, second, first):
        notebook_integration.activate(qtbot, window, note)
        ready(qtbot, window)
        assert window.editor.document() is documents[note]
        assert window.editor.font().family() == "Yu Gothic"
        assert window.editor.document().defaultFont().pointSize() == 19
        assert (
            window.editor.document().firstBlock().layout().boundingRect().height()
            > first_height * 1.5
        )
        assert window.preview.view.zoomFactor() == pytest.approx(1.75)
        assert window.editor.toPlainText() == body
        assert window.editor.document().availableUndoSteps() == undo_steps[note]
    notebook_integration.saved(qtbot, window)
    assert {note: window.store.get(note).revision for note in (first, second)} == revisions
