"""Ephemeral search highlights for snippets, Qt source and preview text."""
...
def preview_highlight_script(
    ...
    *,
    ...
    return _PREVIEW_SCRIPT.replace("__MARKNOTES_PAYLOAD__", payload).replace(
        "__MARKNOTES_TEXT_RUNS__", _PREVIEW_TEXT_RUNS
    )


def preview_search_text_script(revision: int, *, generation: int = 0, note_id: str = "") -> str:
    ...
    bearing the same revision, generation and note ID.
    """
    payload = json.dumps(
        {"revision": int(revision), "generation": int(generation), "noteId": note_id},
        ensure_ascii=True,
    )
    return _NOTE_SEARCH_TEXT_SCRIPT.replace("__MARKNOTES_PAYLOAD__", payload).replace(
        "__MARKNOTES_TEXT_RUNS__", _PREVIEW_TEXT_RUNS
    )
...
def preview_search_ranges_script(
    ...
    *,
    ...
    highlights. At most 5,000 matches are displayed, as in library search.
    """
    ...
    return _NOTE_SEARCH_RANGES_SCRIPT.replace("__MARKNOTES_PAYLOAD__", payload).replace(
        "__MARKNOTES_TEXT_RUNS__", _PREVIEW_TEXT_RUNS
    )
...
_PREVIEW_TEXT_RUNS = r"""
  const collectTextRuns = () => {
    ...
    // Whitespace between block elements is HTML formatting, not note text.
...
(() => {
  ...
  const previous = window.__marknotesNoteSearch;
  if (previous && previous.revision === request.revision) {
    ...
    if (previous.generation === request.generation && previous.noteId !== request.noteId) {
      return {applied: false, reason: "note"};
    }
  }
  if (!window.CSS || !CSS.highlights || !window.Highlight) {
    return {applied: false, reason: "unsupported"};
  }
  __MARKNOTES_TEXT_RUNS__
  const runs = collectTextRuns();
  window.__marknotesNoteSearch = {...request, runs};
  CSS.highlights.delete("marknotes-note-search");
...
(() => {
  ...
  const snapshot = window.__marknotesNoteSearch;
  if (snapshot && snapshot.revision === request.revision) {
    if (snapshot.generation > request.generation) {
      return {applied: false, reason: "generation"};
    }
    if (snapshot.generation === request.generation && snapshot.noteId !== request.noteId) {
      return {applied: false, reason: "note"};
    }
  }
  if (!window.CSS || !CSS.highlights || !window.Highlight) {
  ...
  const name = "marknotes-note-search";
  ...
  if (!snapshot || snapshot.revision !== request.revision ||
      snapshot.generation !== request.generation || snapshot.noteId !== request.noteId) {
  ...
  if (!document.getElementById("marknotes-note-search-highlight-style")) {
    const style = document.createElement("style");
    style.id = "marknotes-note-search-highlight-style";
    style.textContent = "::highlight(marknotes-note-search) {" +
      "background-color:#fff0a8;text-shadow:none;}" +
      ":root[data-theme=dark] ::highlight(marknotes-note-search) {" +
...
(() => {
  ...
  for (const current of runs) {
    ...
    while ((offset = normalized.indexOf(request.query, offset)) >= 0) {
      ...
      lastEnd = end;
      const range = textRunRange(current, start, end);
      if (!range) continue;
      ranges.push(range);
